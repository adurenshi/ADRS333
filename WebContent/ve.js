...
//初始化表格，目前作为通用函数，后期应进行整合
 ...
 this.injectElement = function injElement(id, val)
 ...
 //导出
 this.exportdata= function t_unload(unloadtype,datainfo){//导出文件
	 if(datainfo=="1"){
     var exportsql=document.getElementById("exportsql").value
	 }
	 if(datainfo=="2"){
	     var exportsql=document.getElementById("exportsql2").value
		 }
	 if(datainfo=="3"){
	     var exportsql=document.getElementById("exportsql3").value
		 }
	 if(datainfo=="4"){
	     var exportsql=document.getElementById("exportsql4").value
		 }
	 var form=$("<form>");//定义一个form表单
	 form.attr("style","display:none");
	 form.attr("target","");
	 form.attr("method","post");
	 form.attr("action","unload");
	 var input1=$("<input>");
	 input1.attr("type","hidden");
	 input1.attr("name","exportData");
	 input1.attr("value",exportsql);
	  var input2=$("<input>");
	 input2.attr("type","hidden");
	 input2.attr("name","datatype");
	 input2.attr("value",unloadtype);
	  var input3=$("<input>");
		 input3.attr("type","hidden");
		 input3.attr("name","datainfo");
		 input3.attr("value",datainfo);
 ...
 //导入
 this.importdata= function t_imload(){//导入文件
     var exportsql=document.getElementById("file").files[0];
     alert(exportsql);
	 var form=$("<form>");//定义一个form表单
	 form.attr("style","display:none");
	 form.attr("target","");
	 ...
	 form.attr("action","Xlsimporttodb");
	 var input1=$("<input>");
	 input1.attr("type","file");
	 input1.attr("name","file11");
	 input1.attr("value",exportsql);
	 $("body").append(form);//将表单放置在web中
	 form.append(input1);
	 form.submit();//表单提交	 
...
//表格控件1
 this.myGrid11 = function Grid3(selectortable,tablepage,colNA,colMA){
	 $(selectortable).jqGrid({  
         height: 300,
         autowidth:true,
         mtype: 'POST',
         colNames: colNA,
         ...
         shrinkToFit:true,//宽
			...
			//autowidth:true,
			rownumbers:true, //行号数
			rowList: [100, 150, 210,300],
			//grouping:true,
			/*	gridComplete: function () {//无效，增加某列超链接
		            var graduateIds = jQuery(selectortable).jqGrid('getDataIDs');
		        ...
		        gridComplete:function(){ //在此事件中循环为每一行添加修改和删除链接 
		        	var ids=jQuery(selectortable).jqGrid('getDataIDs'); 
		        	...
		        	jQuery(selectortable).jqGrid('setRowData', ids[i], { Modify: modify, Delete: del }); 
		        	} 
		        	},*/
			//gridComplete: function() { var rowNum = parseInt($(this).getGridParam("records")); if (rowNum <= 0) { alert("没有符合条件的记录！"); } },
			//onSelectRow: function(rowid) {alert(rowid)},//选择行时，执行
			loadonce:true,//关键所在,好像不用？
			userDataOnFooter: true,
			//buttonicon:'ui-icon-newwin',
			caption: "教育卡……"})
			.trigger('reloadGrid', [{page:1}])
.navGrid(tablepage,{edit:true,add:true,del:true});
     //新增下载buttion
/*.navButtonAdd(tablepage,{   
caption:"<a>导出execl</a>",title:"Export Excel",    
...
/*	 .navButtonAdd(tablepage,  
             {caption : "dddd", buttonicon: "ui-icon-folder-open",   
             onClickButton:  
                 function (){    
                     $("#xm").css("display","block");  
                     var actionUrl = "ExportServlet";   
                     $.ajax( {  
                     url : actionUrl,  
                     type:"post",  
                     dataType: "json",  
         ...
         },position:"last", title:"导出"});*/
	//对选中的行进行处理
		this.myGridselectrowsid=function testSelectrowsId(selectortable,_gird_idvalue){
		   ...
		   if(rowData.length) 
		    {
		        ...
		        {
		           var name= jQuery(selectortable).jqGrid('getCell',rowData[i],_gird_idvalue);//name是colModel中的一属性
		           alert(_gird_idvalue+"="+name);
		        }
		    }
		}
		
		//重新载入jqgrid数据
 		this.myreloadGrid=function testquery(){
		jQuery(selectortable).setGridParam({page:1}).trigger("reloadGrid");
		}
	//清除数据
 		this.myclearGridData=function testclear(){
...
//表格控件1
 ...
 this.myGrid = function Grid3(selectortable,tablepage,colNA,colMA,selectarray){
	 selectid=new Array();//保存所选字段id号
	 selx=new Array();//所选行号
	 $(selectortable).jqGrid({  
         height: 300,
         autowidth:true,
         ...
         shrinkToFit:true,//宽
			...
			rownumbers:true, //行号数
			rowList: [100, 150, 210,300],
			loadComplete:function(xhr){//服务器返回时，执行
				for(var i=0;i<selx.length;i++){
				      $(selectortable).jqGrid('setSelection',selx[i]);//选择行
				}
				//alert(selx);
			},
			onSelectAll:function(aRowids,status){//全选执行
				if(status){
				  for(var index=0;index<aRowids.length;index++){
					//alert(aRowids[index]+"===");
					if(selx.indexOf(aRowids[index])==-1){//有则不加入数据
					     selx.push(aRowids[index]);
					     var thisid= $(selectortable).getCell(aRowids[index],selectarray);//根据行号，获取字段id号
					     selectid.push(thisid);
					}
				  }
				}else{
					...
					for(var index=0;index<aRowids.length;index++){
						 var startx=-1;
					     for(var i=0;i < selx.length;i++){
					            if(selx[i]==aRowids[index]){
					            startx=i;
					            break;
					            }
					        }
						selx.splice(startx,1);//删除行号
						selectid.splice(startx,1)//删除字段id号;
			...
			onSelectRow:function(aRowids,status){
				if(status){
					if(selx.indexOf(aRowids)==-1){//有则不加入数据
					     selx.push(aRowids);
					     var thisid= $(selectortable).getCell(aRowids,selectarray);//根据行号，获取字段id号
					     selectid.push(thisid);
					}
				}else{
					var startx=-1;
			...
			caption: "教育卡……"})
			.trigger('reloadGrid', [{page:1}])
.navGrid(tablepage,{edit:false,add:false,del:false});
     //新增下载buttion
/*.navButtonAdd(tablepage,{   
caption:"<a>导出execl</a>",title:"Export Excel",    
buttonicon:"ui-icon-disk",   
	...
	//清除数据
		...
		jQuery(selectortable).jqGrid('clearGridData');
 		...
 		this.deleteContact = function() {
 			 var selectedId = $(selectortable).jqGrid("getGridParam", "selrow");
 			$(selectortable).jqGrid('delRowData', selectedId);
 			alert("删除");
 			};
 		  this.selectidall=function(){//返回所选的id号字段值
 				return selectid;
 			}
}
 
 //芯片表
 this.mychipGrid = function Grid3(selectortable,tablepage,colNA,colMA,selectarray){
	 selectid=new Array();//保存所选字段id号
	 selx=new Array();//所选行号
	 $(selectortable).jqGrid({  
         height: 300,
         ...
         shrinkToFit:true,//宽
			...
			onSelectRow:function(aRowids,status){
				if(status){
					if(selx.indexOf(aRowids)==-1){//有则不加入数据
					     selx.push(aRowids);
					     var thisid= $(selectortable).getCell(aRowids,"cnum");//根据行号，获取字段id号
					     selectid.push(thisid);
					     $("#cnum").val(thisid) ;
					     $("#type").val($(selectortable).getCell(aRowids,"type"));
					     //document.getElementById("cobank").value=$(selectortable).getCell(aRowids,"合作银行");;
					     $("#content").val($(selectortable).getCell(aRowids,"content"));
					     $("#cpu").val($(selectortable).getCell(aRowids,"cpu"));
					     $("#add").attr({"disabled":"disabled"});
					     $('#cnum').attr('disabled',"true");
					     $("#add").css("background-color","#666");
					     //$("#add").attr("style",{"background-color":"#666"});
					}
				}else{
					var startx=-1;
				     for(var i=0;i < selx.length;i++){
					...
					document.getElementById("cnum").value="";
				     ...
				     $('#cnum').removeAttr("disabled");
				     $('#add').removeAttr("disabled"); 
				     $("#add").css("background-color","#007fff");
				     $('#add').hover(function() {
				    	 $("#add").css("background-color","#139257");
				     });
...
.navGrid(tablepage,{edit:false,add:false,del:false});
 ...
 this.myGridmodi = function Grid36(selectortable,tablepage,colNA,colMA){
	 selectid=new Array();//保存所选字段id号
	 selx=new Array();//所选行号
	 $(selectortable).jqGrid({  
         height: 300,
         ...
         shrinkToFit:true,//宽
			...
			onSelectAll:function(aRowids,status){//全选执行
				if(status){
				  for(var index=0;index<aRowids.length;index++){
					...
					if(selx.indexOf(aRowids[index])==-1){//有则不加入数据
					     selx.push(aRowids[index]);
					     var thisid= $(selectortable).getCell(aRowids[index],"id号");//根据行号，获取字段id号
					     selectid.push(thisid);
					}
				  }
				}else{
			...
			onSelectRow:function(aRowids,status){
				if(status){
					if(selx.indexOf(aRowids)==-1){//有则不加入数据
					     selx.push(aRowids);
					     var thisid= $(selectortable).getCell(aRowids,"foreid");//根据行号，获取字段id号
					     selectid.push(thisid);
					     document.getElementById("idxx").value=thisid;
					     document.getElementById("ssn").value=$(selectortable).getCell(aRowids,"ssn");
					     //document.getElementById("cobank").value=$(selectortable).getCell(aRowids,"合作银行");;
					     document.getElementById("cobank").value=$(selectortable).getCell(aRowids,"co_bank_cardno");
					     document.getElementById("personcardid").value=$(selectortable).getCell(aRowids,"card_serial_number");
					     document.getElementById("bankpassword").value=$(selectortable).getCell(aRowids,"initial_password");
					     var handid = $(selectortable).getCell(aRowids,"handid");
					     //$("#handid").empty();
					     //$("#handid").append("<option value="+ handid + ">"+ handid+ "</option>");
					     //$($("#handid option")[0]).text(handid);
					     $("#handid option[selected]").text(handid);
					}
				}else{
					var startx=-1;
				     for(var i=0;i < selx.length;i++){
					...
					selx.splice(startx,1);//删除行号
					selectid.splice(startx,1)//删除字段id号;
					document.getElementById("idxx").value="";
				     document.getElementById("ssn").value="";
				     document.getElementById("cobank").value="";
				     document.getElementById("personcardid").value="";
				     document.getElementById("bankpassword").value="";
				     document.getElementById("handid").value="";
...
function Utilities()
{
 this.Cookie = {
	...
	this.getMsgMapByCookie=function()
	{//从Cookie读取消息映射表
	 var msgMap = cVeUti.Cookie.getCookie('VeMsgMap');
	 alert("得到cookies:"+msgMap);
