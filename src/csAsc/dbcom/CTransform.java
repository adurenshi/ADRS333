...
public class CTransform 
{
 ...
 public static StringBuffer RsToArrayStr1(ResultSet rs) throws SQLException 
 {//将ResultSet型的数据串行化为字符串（行序）：每条记录是一个数组元素。由各个字段的数组组成
  //每行的字段用逗号分隔，每行用一对方括号括起来，行间用逗号分隔
  ResultSetMetaData rsmd=rs.getMetaData();
  int k = rsmd.getColumnCount(); //列数
  ...
  while (rs.next())
  {
   ...
   tRowBuffer.append(rsmd.getColumnName(1)+":\""+rs.getString(1)+"\"") ;
   for (int i = 2; i <= k; i++)
   {
	//System.out.println(rs.getString(i));
	tRowBuffer.append( ","+rsmd.getColumnName(i)+":\""+rs.getString(i)+"\"");
   }
   tRecsBuffer.append("{"+tRowBuffer+"},");
   tRowBuffer.setLength(0);
  }
