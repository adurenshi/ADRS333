...
{//基于Servlet的消息引擎	
 ...
 public class CParam
 {
  ...
  public String getRetMsgHead(String reqVeName,String reqMsgType, String reqMsgName) 
  {	 
   return  param.getReqVeNameId()+":\""+ param.getReqVeName() +"\"," 
     + param.getReqMsgTypeId()+":\""+ param.getReqMsgType() + "\"," + param.getReqMsgNameId()+":\""+ param.getReqMsgName() + "\"";
  } 
 }
 ...
 {//初始化程序，在Servlet生命周期内仅执行一次
  ...
  String  appConfigFile = config.getInitParameter("AppConfigFile");//获取用户文件的绝对路径（即WebContent的绝对路径）
  param.setConfigFile(appConfigFile);//获取VE应用的配置文件的路径
  String file=getClass().getClassLoader().getResource("EIO.xml").getFile();
  param.eioConfigObj = new CEioConfig(file);
  try
  {
   //System.out.println(param.eioConfigObj.getXml2Json("消息包2"));
   param.setReqMsgTypeId(param.eioConfigObj.getInitParameter("VEReqMsgTypeId"));//获取VE消息的种类的标识符
   param.setReqMsgNameId(param.eioConfigObj.getInitParameter("VEReqMsgNameId"));//获取VE消息的名称的标识符
   param.setReqVeNameId(param.eioConfigObj.getInitParameter("VeNameId"));//获取VE消息的名称的标识符

   
  }catch (Exception e)
   {System.out.println("读配置文件错误"); e.printStackTrace();} 
 ...
 // param.setMsgHandle(config.getInitParameter(param.getReqMsgName()));//获取VE消息的种类的标识符
  ...
  if (param.getReqMsgType()==null || param.getReqMsgName()==null ) 
	{
	 System.out.println("消息头类型或者名称为空:"+param.getReqMsgType()+" "+param.getReqMsgName()+"  "+param.getReqMsgTypeId()+" "+param.getReqMsgNameId()) ;
	 return; 
	}
  
  if ( param.getMsgHandle()==null) 
	{
	 System.out.println("信息名称对应的消息处理器映射未发现，请检查配置文件:"+param.getReqMsgType()+" "+param.getReqMsgName()) ;
	 return; 
	}  
  
  String msgHandle = param.getMsgHandle();
  System.out.println("准备调用的消息处理器=="+msgHandle+"  消息名称="+param.getReqMsgName());
  
  String retMsghead = param.getRetMsgHead(param.getReqVeName(),param.getReqMsgType(),param.getReqMsgName());  
  //StringBuffer respData=new StringBuffer("{"+retMsghead+",\""+param.getReqMsgDataId()+"\":");
  
  param.respData = new StringBuffer("{"+retMsghead+",\""+param.getReqMsgDataId()+"\":");//每个消息对应一个新的retData

  int k = msgHandle.lastIndexOf(".");
  switch (msgHandle.substring(k+1,msgHandle.length())) //根据消息dying的不同处理器进行处理
  {
   case "handleMsg":
	 callMsgHandle(param);
   ...
   case "handleMsgHttp1":
		 ...
		 sendRespDada();
		 break;
   case "handleMsgHttpsubpackUpdate":
	     callMsgHandle(param);
	     param.respData.append("}");
		 sendRespDada();
		 break;  
 ...
 public int  callMsgHandle(CParam param)
	     throws ServletException,IOException
