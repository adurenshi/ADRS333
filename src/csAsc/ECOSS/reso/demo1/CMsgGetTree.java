...
public class CMsgGetTree
{
			...
			SQLException, Exception
	{
		...
		// String treeName = msgDataObj.getString("TreeName"); //
		// 从消息中取出消息内容：TreeName的值\

		String treeRange = msgDataObj.getString("TreeRange");
		 ...
		 * System.out
		 * .println("5="+msgReq.getSession().getServletContext().getRealPath
		 * ("/place.xml"));
		 */
		...
		// String
		// filePath=smsgReq.getSession().getServletContext().getRealPath("/place.xml");

		//String projectPath = param.getProjectPath();
		String treeFile = getClass().getClassLoader().getResource("place.xml")
				.getFile();
		if (treeRange.equals(""))
		{
			// "C:\\place.xml"
			param.respData.append(XMLParser.getData(treeFile));
		} else
		{
			param.respData.append(XMLParser.getData(treeFile, treeRange));
		}
		...
		/****** ********************************************/
		/**
		 * String sqlStr; ResultSet recs; sqlStr =
		 * "select area_id, area_name, parent_id from website_area"; recs =
		 * CQuery.getRecsBySql(sqlStr); retData = CTransform.RsToArrayStr(recs);
		 * System.out.println("返回的数据内容："+retData); return retData;
		 */
